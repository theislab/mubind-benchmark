...
if __name__ == '__main__':
    ...
    metrics = []
    for (n_epochs, lr, batch_size, n_kernels) in itertools.product(args.n_epochs, args.learning_rates, args.batch_sizes, args.n_kernels):
    ...
    # train_test = tdata.DataLoader(dataset=dataset, batch_size=1, shuffle=False)                
            ...
            metrics.append(list(r.values[:-1]) + [batch_size, lr, n_epochs, n_kernels, model.best_loss, r2, model.total_time])
            
        metrics = pd.DataFrame(metrics, columns=list(queries.columns[:-1]) + ['batch_size', 'learning_rate', 'n_epochs', 'n_kernels', 'best_loss', 'r_2', 'running_time'])
