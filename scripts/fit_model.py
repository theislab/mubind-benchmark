    ...
    for n_epochs in args.n_epochs:
        for lr in args.learning_rates:
            for batch_size in args.batch_sizes:
                hyperparam_id = f'N_{n_epochs}_L_{lr}_B_{batch_size}'
        ...
        # train_test = tdata.DataLoader(dataset=dataset, batch_size=1, shuffle=False)                
                    ...
                    if not exists(model_path):
                        ...
                        model, best_loss = mb.tl.train_iterative(train, device, num_epochs=n_epochs, show_logo=False,
                                                                early_stopping=args.early_stopping, log_each=50, lr=lr)
                    ...
                    for idx, val in enumerate(model.r2_history):
                        metrics.append(list(r.values[:-1]) + [batch_size, lr, idx, -1, val, -1])

                    metrics.append(list(r.values[:-1]) + [batch_size, lr, n_epochs, model.best_loss, r2, model.total_time])
                    
                metrics = pd.DataFrame(metrics, columns=list(queries.columns[:-1]) + ['batch_size', 'learning_rate', 'n_epochs', 'best_loss', 'r_2', 'running_time'])
